    ...
    @foreach ($reputation as $reputation_item)
        ...
        <div class="modal-window__list-item reputation-history__item" data-id="{{$reputation_item->id}}">
            <div class="reputation-history__item__top">
                <a @if ($reputation_item->from) target="_blank" href="{{$reputation_item->from->url}}"
                   @endif class="reputation-history__item__user">{{$reputation_item->from ? $reputation_item->from->username : "DELETED"}}</a>
                <span
                    class="reputation-history__item__weight @if ($reputation_item->weight > 0) reputation-history__item__weight--positive @elseif ($reputation_item->weight < 0) reputation-history__item__weight--negative @else reputation-history__item__weight--neutral @endif">
                {{$reputation_item->weight > 0 ? "+".$reputation_item->weight  : $reputation_item->weight }}
            </span>
                <span
                    class="reputation-history__item__date">{{$reputation_item->created_at->format('d.m.Y H:i:s')}}</span>
                @if ($reputation_item->link)
                    <a href="{{$reputation_item->link}}" target="_blank" class="reputation-history__item__source">[Источник]</a>
                @endif
            ...
            @if ($can_edit_reputation)
                <form data-callback="editReputationCallback" data-noscroll="1" method="POST"
                      class="form reputation-history__item__form" action="{{route('reputation.edit')}}"
                      style="display: none">
                        ...
                        <div class="input-container input-container--vertical">
                            <div class="input-container__inner">
                                <textarea class="input input--textarea"
                                          name="comment">{{$reputation_item->comment}}</textarea>
                            </div>
                        </div>
            ...
            @endif
            @if ($can_reply)
                <form data-callback="replyReputationCallback" data-noscroll="1" method="POST"
                      class="form reputation-history__item__reply-form" action="{{route('reputation.reply')}}"
                      style="display: none">
                    <div class="form__content">
                        <input type="hidden" name="id" value="{{$reputation_item->id}}"/>
                        <div class="input-container input-container--vertical">
                            <div class="input-container__inner">
                            <textarea class="input input--textarea"
                                      name="reply_comment">{{$reputation_item->reply_comment}}</textarea>
                            </div>
                        </div>
                        <div class="form__bottom">
                            <button class="button button--light">ОК</button>
            ...
            <div class="reputation-history__item__reply-comment"
                 @if ($reputation_item->reply_comment == "") style="display: none" @endif>
                <strong>Ответ: </strong><span
                    class="reputation-history__item__reply-comment__text">{{$reputation_item->reply_comment}}</span>
            </div>
        </div>
    @endforeach
</div>
