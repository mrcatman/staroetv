...
@foreach ($reputation as $reputation_item)
    ...
    <div class="modal-window__list-item reputation-history__item" data-id="{{$reputation_item->id}}">
        ...
        @if ($can_edit_reputation)
        <form data-callback="editReputationCallback" data-noscroll="1" method="POST" class="form reputation-history__item__form" action="{{route('reputation.edit')}}" style="display: none">
        ...
        @endif
        @if ($can_reply)
            <form data-callback="replyReputationCallback" data-noscroll="1" method="POST" class="form reputation-history__item__reply-form" action="{{route('reputation.reply')}}" style="display: none">
                <input type="hidden" name="id" value="{{$reputation_item->id}}"/>
                <div class="input-container input-container--vertical">
                    <div class="input-container__inner">
