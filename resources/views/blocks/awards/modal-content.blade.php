<div class="awards-history">
    ...
    @if (count($awards) == 0)
        <div class="modal-window__nothing-found">У этого пользователя нет наград</div>
    @endif
        ...
        <div class="awards-history__item modal-window__list-item" data-id="{{$award->id}}">
            ...
            <div class="awards-history__item__texts">
                <div class="awards-history__item__top">
                    <a @if ($award->from) target="_blank" href="{{$award->from->url}}"
                       @endif class="awards-history__item__user">{{$award->from ? $award->from->username : "DELETED"}}</a>
                ...
                @if ($can_edit_awards)
                    <form data-callback="editAwardCallback" data-noscroll="1" method="POST"
                          class="form awards-history__item__form" action="{{route('awards.edit')}}"
                          style="display: none">
