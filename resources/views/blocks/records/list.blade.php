                    ...
                    <div class="records-list__sort">
                        <div class="top-list records-list__sort__items">
                            <a class="top-list__item @if ($records_data['sort'] == "newer") top-list__item--active @endif"
                               href="{{$records_data['base_link']}}?{{http_build_query(array_merge(\App\Helpers\ArraysHelper::diffAssoc($records_data['query_params'], ['conditions', 'sort']), ['sort' => 'newer']))}}">От
                                новых к старым</a>
                            <a class="top-list__item @if ($records_data['sort'] == "older") top-list__item--active @endif"
                               href="{{$records_data['base_link']}}?{{http_build_query(array_merge(\App\Helpers\ArraysHelper::diffAssoc($records_data['query_params'], ['conditions', 'sort']), ['sort' => 'older']))}}">От
                                старых к новым</a>
                            <a class="top-list__item @if ($records_data['sort'] == "added") top-list__item--active @endif"
                               href="{{$records_data['base_link']}}?{{http_build_query(array_merge(\App\Helpers\ArraysHelper::diffAssoc($records_data['query_params'], ['conditions', 'sort']), ['sort' => 'added']))}}">Недавно
                                добавленные</a>
                        </div>
                    ...
                    @if ($records_data['months'])
                        ...
                        <div class="top-list records-list__months">
                            <a class="top-list__item top-list__item--all @if (!$records_data['selected_month']) top-list__item--active @endif"
                               href="{{$records_data['base_link']}}?{{http_build_query(\App\Helpers\ArraysHelper::diffAssoc($records_data['query_params'], ['conditions', 'month']))}}">
                            ...
                            @foreach ($records_data['months'] as $month => $count)
                                @if (isset($month_names[$month - 1]))
                                    <a class="top-list__item @if ($records_data['selected_month'] == $month) top-list__item--active @endif"
                                       href="{{$records_data['base_link']}}?{{http_build_query(array_merge(\App\Helpers\ArraysHelper::diffAssoc($records_data['query_params'], ['conditions', 'month']), ['month' => $month]))}}">
