@if ($can_edit)
    ...
    <!--
@if ($article->type_id != \App\Models\Article::TYPE_ARTICLES)
    ...
    @endif
    @if ($article->type_id != \App\Models\Article::TYPE_NEWS)
    ...
    @endif
    @if ($article->type_id != \App\Models\Article::TYPE_BLOG)
...
@endif
@if ($can_approve)
    <a class="button--dropdown__list__item" data-approve="articles"
       data-approve-id="{{$article->id}}">{{$article->pending ? "Одобрить" : "Скрыть"}}</a>
@endif
