...
<div class="box box--dark" data-id="{{$questionnaire->id}}">
    @if ($questionnaire->title != '')
        <div class="box__heading">
            <div class="box__heading__inner">{{$questionnaire->title}}</div>
        </div>
    @endif
    <div class="box__inner">
        @if (!isset($show_results) || !$show_results)
            <form data-noscroll="1" action="{{route('forum.questionnaire.vote')}}" class="form questionnaire__variants">
                @csrf
                <input type="hidden" name="questionnaire_id" value="{{$questionnaire->id}}">
                @foreach ($questionnaire->variants as $variant)
                    <div class="questionnaire__variant">
                        <label
                            class="input-container @if($questionnaire->multiple_variants) input-container--checkbox @else input-container--radio @endif">
        ...
        <div class="questionnaire__results">
            @foreach ($questionnaire->variants as $variant)
                <div class="questionnaire__result">
                    ...
                    <div class="questionnaire__result__bar"
                         style="width: {{$variant->answers_count === 0 ? 0 : $variant->answers_count / $questionnaire->total_answers * 100}}%"></div>
                    <div
                        class="questionnaire__result__percent">{{ $variant->answers_count === 0 ? 0 : number_format($variant->answers_count / $questionnaire->total_answers * 100, 2, '.', '')}}
                        %
                    </div>
                </div>
            @endforeach
...
<div class="questionnaire__bottom">
    ...
    @if (auth()->user())
        <form class="form" action="{{route('forum.questionnaire.form')}}">
            @csrf
            <input type="hidden" name="questionnaire_id" value="{{$questionnaire->id}}"/>
            @if (!isset($show_results) || !$show_results)
                <input type="hidden" name="show_results" value="1"/>
                <button class="button button--light">Посмотреть результаты</button>
            @else
                <input type="hidden" name="show_results" value="0"/>
                <button class="button button--light">Переголосовать</button>
            @endif
        </form>
    @endif
</div>
