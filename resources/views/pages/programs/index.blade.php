        ...
        <div class="box__inner">
            <div class="channel-page__programs">
                <div class="categories-list">
                    <a class="category @if (!$category) category--active @endif" href="{{typed_route('records.[RECORD].programs', $is_radio)}}">
                        Все
                    </a>
                    @foreach($categories as $category_item)
                        <a class="category @if ($category && $category_item->id == $category->id) category--active @endif" href="{{typed_route('records.[RECORD].programs', $is_radio, ['category' => $category_item->url])}}">
                            {{$category_item->name}}
                        </a>
                    @endforeach
                </div>
                ...
                <div class="programs-list__container">
                    <div
                        ...
                        @include('blocks.programs.list', ['programs' => $programs])

                    </div>
                    @if ($show_load_more_button && count($records_conditions['program_id_in']) > 20)
                        <div class="programs-list__show-all">
                            <a data-is-radio="{{$params['is_radio']}}" data-category="{{$category ? $category->url : null}}" data-period="{{$period ? $period['url'] : null}}" class="button">Показать все</a>
                        </div>
                    @endif
                </div>
