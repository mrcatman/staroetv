            ...
            <div class="users-top">
                <div class="users-top__links">
                    @foreach ($links as $url => $name)
                        <a class="users-top__link @if (request()->path() == 'top-list/'.$url) users-top__link--active @endif"
                           href="/top-list/{{$url}}">{{$name}}</a>
                    @endforeach
                </div>
                <div class="users-top__values">
                    @foreach($list as $index => $row)
                        <div class="users-top__row">
                            ...
                            <a target="_blank" class="users-top__row__user"
                               href="{{$row['user']->url}}">{{$row['user']->username}}</a>
                            @if ($row['user']->group)
